...
export const updateAdvisorProfile = async (advisorData: any) => {
  ...
  const { id, created_at, updated_at, profile_img, ...updateData } =
    advisorData;
...
export const uploadAdvisorImage = async (formData: FormData) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Keep file error (might be normal if folder exists):",
        keepError
      );
    }
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("Advisors").getPublicUrl(filePath);
...
export const updateProfileImage = async (
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("Advisors").getPublicUrl(imagePath);
